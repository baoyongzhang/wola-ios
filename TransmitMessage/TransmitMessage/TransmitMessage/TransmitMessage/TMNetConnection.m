...
{
    NSString *reqStr = [req createJson];
    [self asyncPostRequest:[TMConfig getCenterURl] withData:[reqStr dataUsingEncoding:NSUTF8StringEncoding] complete:^(NSString *responseStr){
        // 请求成功
        SBJsonParser *jsonParser = [[SBJsonParser alloc] init];
        NSMutableDictionary *dict = [jsonParser objectWithString:responseStr];
        // 解析为Response对象
        [jsonParser release];
    } failed:^(int code) {
        // 请求失败
